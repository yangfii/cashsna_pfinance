...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 overflow-hidden relative group", // enhanced transitions
  {
    variants: {
      variant: {
        ...
        outline:
          "border border-border bg-background text-foreground hover:bg-accent hover:text-accent-foreground hover:scale-105 hover:shadow-md active:scale-95",
        secondary:
        ...
        google:
          "bg-card text-card-foreground border border-border hover:bg-muted shadow-sm hover:shadow-md hover:scale-105 active:scale-95",
  ...
  ({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        {...props}
      >
        <span className="truncate overflow-hidden text-ellipsis">{children}</span>
      </Comp>
    )
  }
)
