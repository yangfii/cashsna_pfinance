...
export default function ExchangeIntegration({ onImportHoldings }: ExchangeIntegrationProps) {
  ...
  const connectBinance = async () => {
    if (!binanceApiKey || !binanceSecret || !binanceAccountName) {
      toast.error("Please enter account name, API key and secret");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        await saveAccount('binance', binanceAccountName, binanceApiKey, binanceSecret);
      ...
      if (error && typeof error === 'object') {
        ...
        } else if (errorObj.message?.includes('Signature')) {
          errorMessage = "Invalid signature. Please check your secret key.";
        } else if (errorObj.message?.includes('IP')) {
          errorMessage = "IP restriction error. Please check your API key IP whitelist settings.";
        }
      }
  ...
  const connectBybit = async () => {
    ...
    try {
      ...
      if (data?.success) {
        await saveAccount('bybit', bybitAccountName, bybitApiKey, bybitSecret);
      ...
      let errorMessage = "Failed to connect to Bybit. Please check your credentials.";
      if (error && typeof error === 'object') {
        const errorObj = error as any;
        if (errorObj.message?.includes('Invalid API key')) {
          errorMessage = "Invalid API key. Please verify your Bybit API key is correct.";
        } else if (errorObj.message?.includes('Invalid signature')) {
          errorMessage = "Invalid signature. Please check your secret key.";
  ...
  const syncAccount = async (account: ExchangeAccount) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        toast.success(`Successfully synced ${account.exchange_name} account`);
      } else {
      ...
      toast.error(`Failed to sync ${account.exchange_name} account: ${errorMessage}`);
    } finally {
      setIsSyncing(false);
    }
  };
